...
根据 KBPD Admin 项目宪法（版本 2.0.0），所有功能必须验证以下原则：

### 1. Vue 最佳实践原则
- [ ] 所有组件使用 Vue 3 Composition API 和 `<script setup>` 语法
- [ ] Props/Emits 具有显式类型定义
- [ ] 共享逻辑提取到 composables（不使用 mixin）

### 2. 类型安全原则（不可协商）
- [ ] TypeScript strict 模式已启用
- [ ] 所有函数具有显式返回类型
- [ ] 无 `any` 类型使用（使用 `unknown` 配合类型守卫）

### 3. 组件可复用性原则
- [ ] 组件自包含（依赖仅通过 props/emits）
- [ ] 功能代码在 `src/features/[feature]/` 下
- [ ] 共享 UI 组件在 `src/components/ui/` 下

### 复杂性例外
如需违反上述原则，必须在下方记录：

| 原则 | 为什么需要例外 | 更简单的替代方案及拒绝理由 |
|------|--------------|--------------------------|
| [例如：使用第三方库而非自定义 composable] | [具体原因] | [为什么更简单方案不足] |
