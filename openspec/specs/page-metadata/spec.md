...
系统 SHALL 提供集中的路由配置文件 `src/routes/config.ts`，包含每个路由的标题 i18n key 和面包屑层级。

#### Scenario: 配置文件包含所有路由元数据

- **WHEN** 应用启动
- **THEN** `routeConfig` 导出包含所有主要路由的配置对象，每个配置包含 `path`、`titleKey`、`breadcrumb` 属性

#### Scenario: 路由配置与布局 i18n key 一致

- **WHEN** 开发者定义路由元数据
- **THEN** `titleKey` 必须使用 `layout.*` namespace 的 i18n key（如 `layout.dashboard`、`layout.system.users`）
...
系统 SHALL 提供 `useBreadcrumbs()` Hook，根据当前路由和配置自动生成面包屑路径。
...
- **WHEN** 用户访问 `/system/users/roles`
- **THEN** 面包屑显示"首页 / 系统管理 / 用户管理 / 角色管理"
